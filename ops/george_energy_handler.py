...
def handle_energy_scan_completed(event):
    ...
    model_confidence = data.get("model_confidence", "UNKNOWN")
    ...
    if data_confidence == "LOW":
        ...
        reason = "Data confidence is LOW. Baseline validation required before operational recommendation."

    elif overall_traffic == "RED":
        verdict = "BLOCK"
        action = "ESCALATE"
        reason = "Overall judgement is RED. Human review required."

    elif saving_pct < 2:
        verdict = "BLOCK"
        action = "REQUEST_BASELINE"
        reason = f"Estimated saving potential is below threshold: {saving_pct}%."

    elif cap_applied:
        verdict = "RECOMMEND"
        action = "SHAPE_LOAD"
        reason = "Saving potential reached the BIW benchmark cap. Recommend controlled pilot validation."

    else:
        verdict = "RECOMMEND"
        action = "PILOT_REVIEW"
        reason = f"Energy scan indicates {saving_pct}% estimated saving potential."
    ...
    decision = {
        ...
        "evidence": {
            ...
            "dataset_days": data.get("dataset_days"),
            "rows": data.get("rows"),
