# pr/567.decision_trace.md

## Decision / Intent
Enable merge of PR #567 by aligning registry/status/decision-trace requirements and updating the PR decision trace validator to accept *.decision_trace.md.

## Authority
- Class: Governance / CI Compliance
- Authority level: Supervised
- Escalation: If checks fail -> BLOCK (no merge).

## Scope (files/modules touched)
- (list exactly what this PR changes)

## Expected outcome
- All required checks green.
- Status Agent / Consistency Agent onboarding can be merged without bypassing governance.
