/* assets/agent-status.js (1:1 replacement)
...
========================= */
if (window.__agentStatusInit) {
  // If the script is accidentally included twice, do nothing.
  // (Important for avoiding double polling intervals.)
} else {
  window.__agentStatusInit = true;
  ...
  /* =========================
     DOM targets (robust)
     - Prefer header container (correct contract)
     - Fall back to first #agent-chips if needed
  ========================= */
  function ensureContainer() {
    const inHeader = document.querySelector("header #agent-chips");
  ...
  function init() {
    ...
    // Ensure we set only one interval
    if (!window.__agentStatusInterval) {
      window.__agentStatusInterval = window.setInterval(bootOnce, REFRESH_MS);
    }
  }
