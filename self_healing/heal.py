...
def gh_set_output(key: str, value: str) -> None:
    ...
    IMPORTANT: supports multiline values via delimiter syntax.
    """
    out = os.environ.get("GITHUB_OUTPUT")
    if not out:
        return

    # Always use multiline-safe format (works for single-line too)
    delim = "__VIRTAUTO_EOF__"
...
def cleanup_non_governed_noise() -> None:
    ...
    """
    # egg-info (common case: "virtauto.egg-info")
    try:
    ...
    # common cache dirs anywhere under repo
...
# R5: deterministic latest.json regeneration (canonical pointer)
def regenerate_latest_json() -> bool:
    """
    Ensures ops/reports/latest.json exists and contains the canonical pointers.

    Returns True if the file was created/updated, else False.
    """
    ...
    canonical = {
        ...
        "note": "regenerated by self-healing (R5).",
    }

    # If file exists and already matches (ignoring generated_at), do nothing
    if p_latest.exists() and is_valid_json_file(p_latest):
        try:
            current = read_json(p_latest)
            cur_cmp = dict(current) if isinstance(current, dict) else {}
            can_cmp = dict(canonical)

            # Ignore generated_at differences for "already ok" check
            cur_cmp.pop("generated_at", None)
            can_cmp.pop("generated_at", None)

            if cur_cmp == can_cmp:
                return False
        except Exception:
            # fall through to overwrite with canonical
            pass

    write_json(p_latest, canonical)
    return True
...
def main() -> int:
    ...
    # R5: ensure latest.json is present & canonical (deterministic pointers)
    if regenerate_latest_json():
        changed.append(safe_rel(OPS_REPORTS_DIR / "latest.json"))

    # R5: cleanup non-governed noise deterministically (prevents egg-info etc.)
